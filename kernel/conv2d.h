#ifndef __LD_UNIAD_CONV2D_H__
#define __LD_UNIAD_CONV2D_H__
...
#ifdef UNIAD_CONV2D_DEBUG
    printf("[conv2d_fwd]: (%d, %d, %d, %d)\n", N, C_out, H_out, W_out);
    printf("[");
    for (int n = 0; n < N; n++) {
        printf("[");
...
//     }
// #ifdef UNIAD_CONV2D_DEBUG
//     printf("conv2d_fwd: (%d, %d, %d, %d) padding: %d\n", N, C_out, H_out, W_out, padding);
//     printf("[");
//     for (int n = 0; n < N; n++) {
//         printf("[");
//         for (int c = 0; c < C_out; c++) {
...
void conv2d_fwd(float * output, int * output_shape, float * input, int * input_shape,
                float * filters, int * filters_shape, float * bias, 
                int * strides, int * paddings, const char * padding_mode) {
#if defined(UNIAD_BENCHMARK) || defined(UNIAD_CONV2D_BENCHMARK)
    double tdata = omp_get_wtime();
#endif // UNIAD_BENCHMARK
    cc_conv2d_fwd(output, output_shape, input, input_shape, filters, filters_shape, bias, strides, paddings, padding_mode);
#if defined(UNIAD_BENCHMARK) || defined(UNIAD_CONV2D_BENCHMARK)
    tdata = omp_get_wtime() - tdata;
    printf("[benchmark][conv2d_fwd][cc_conv2d_fwd]: in_shape:(%d, %d, %d, %d), weight_shape:(%d, %d, %d, %d), out_shape:(%d, %d, %d, %d), "
