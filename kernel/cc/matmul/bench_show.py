...
def benchmark_matmul():
    ...
    ax.plot(plt_data_x, np_gflops, 'bo-.', label='np.matmul')
    ax.plot(plt_data_x, torch_gflops, 'r-*', label='torch.matmul')
    ax.plot(plt_data_x, blas_gflops, 'g-x', label='blas.matmul')
    ax.set_xticks(plt_data_x[::1])
    ax.tick_params(axis='x', labelrotation=90)
    fig.tight_layout()
    ax.set(xlabel='MNK', ylabel='GFLOPS/sec.', title="Matmul Benchmark")
    ax.grid()
    ax.legend()
