#ifndef __LD_UNIAD_FUSE_CONV2D_BATCH_NORM2D_RELU_H__
#define __LD_UNIAD_FUSE_CONV2D_BATCH_NORM2D_RELU_H__
...
// format NCHW
void cc_fuse_conv2d_batch_norm2d_relu_fwd(float * output, int * output_shape, float * input, int * input_shape,
...
#ifdef UNIAD_FUSE_CONV2D_BATCH_NORM2D_RELU_DEBUG
    printf("fuse_conv2d_batch_norm2d_relu_fwd: (%d, %d, %d, %d)\n", N, C_out, H_out, W_out);
    printf("[");
    for (int n = 0; n < N; n++) {
        printf("[");
    ...
    printf("]\n");
#endif // UNIAD_FUSE_CONV2D_BATCH_NORM2D_RELU_DEBUG
}

void fuse_conv2d_batch_norm2d_relu_fwd(float * output, int * output_shape, float * input, int * input_shape,
                float * filters, int * filters_shape, float * bias, 
                int * strides, int * paddings, const char * padding_mode,
                const float * gamma, const float * beta, const float * mean, const float * var) {
#if defined(UNIAD_BENCHMARK) || defined(UNIAD_FUSE_CONV2D_BATCH_NORM2D_RELU_BENCHMARK)
    double tdata = omp_get_wtime();
#endif // UNIAD_BENCHMARK
    cc_fuse_conv2d_batch_norm2d_relu_fwd(output, output_shape, input, input_shape, filters, filters_shape, bias, strides, paddings, padding_mode,
                                    gamma, beta, mean, var);
#if defined(UNIAD_BENCHMARK) || defined(UNIAD_FUSE_CONV2D_BATCH_NORM2D_RELU_BENCHMARK)
    tdata = omp_get_wtime() - tdata;
    printf("[benchmark][fuse_conv2d_batch_norm2d_relu_fwd][cc_fuse_conv2d_batch_norm2d_relu_fwd]: in_shape:(%d, %d, %d, %d), weight_shape:(%d, %d, %d, %d), out_shape:(%d, %d, %d, %d), "
