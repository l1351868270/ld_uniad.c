#ifndef __LD_UNIAD_CUDNN_CONV2D_NHWC_BEST_H__
#define __LD_UNIAD_CUDNN_CONV2D_NHWC_BEST_H__
...
namespace bench {
namespace cudnn_conv2d_nhwc_best {

template <typename T>
void cuda_cudnn_conv2d_nhwc_best(cudnnHandle_t * handle,
                       const void *alpha,
...
double cudnn_conv2d(T * y, T * x, const T * w, int N, int H, int W, int C, 
    ...
    cuda_cudnn_conv2d_nhwc_best<T>(&handle, &alpha, &xDesc, x, &wDesc, w, &convDesc, &best_algo.algo, workSpace, workSpaceSizeInBytes, &beta, &yDesc, y);
...
#endif // __LD_UNIAD_CUDNN_CONV2D_NHWC_BEST_H__