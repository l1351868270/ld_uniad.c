...
double py_cudnn_frontend_conv2d(torch::Tensor y, torch::Tensor x, torch::Tensor w, int pad_h, int pad_w, int U, int V, int dilation_h, int dilation_w) {
    c10::Half *y_ptr = y.data_ptr<c10::Half>();
    c10::Half *x_ptr = x.data_ptr<c10::Half>();
    c10::Half *w_ptr = w.data_ptr<c10::Half>();
    ...
    // printf("N:%d, H:%d, W:%d, C:%d, K:%d, R:%d, S:%d, pad_h:%d, pad_w:%d, U:%d, V:%d, dilation_h:%d, dilation_w:%d\n", 
    //         N, H, W, C, K, R, S, pad_h, pad_w, U, V, dilation_h, dilation_w);
    return bench::cudnn_frontend_conv2d::cudnn_conv2d<half>(y_hf, x_hf, w_hf, N, H, W, C, 
                       K, R, S, pad_h, pad_w, U, V, dilation_h, dilation_w);
    return 0;
}
...
m.def("cudnn_frontend_conv2d", torch::wrap_pybind_function(py_cudnn_frontend_conv2d), "cudnn_frontend_conv2d");
}