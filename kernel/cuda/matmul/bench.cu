...
double py_kittens_gemm(torch::Tensor C, torch::Tensor A, torch::Tensor B) {
    auto C_data = C.data_ptr<float>();
    auto A_data = A.data_ptr<float>();
    auto B_data = B.data_ptr<float>();
    auto M = C.size(0);
    auto N = C.size(1);
    auto K = A.size(1);
    return kittens_gemm(C_data, A_data, B_data, M, N, K);
}
...
m.def("kittens_gemm", torch::wrap_pybind_function(py_kittens_gemm), "kittens_gemm");
}