#ifndef __LD_UNIAD_CUTLASS_GEMM_H__
#define __LD_UNIAD_CUTLASS_GEMM_H__
...
float cutlass_gemm(float * C, const float * A, float * B, const int M, const int N, const int K) {
    cudaEvent_t start, end;
    cudaEventCreate(&start);
    cudaEventCreate(&end);
    cudaEventRecord(start, 0);

    gemm_tn(N, M, K, 1.0, B, K, A, K, 0.0, C, N, 0);
